...
//! The `--tracing` flag helps user to inspect thread's scan process. It has these levels:
...
/// Trace information for a specific granularity level.
...
/// Trace information for a project, run, or scan.
...
impl<W: Write + Sync> TraceInfo<FileTrace, W> {
  /// Print the summary of the run trace.
  pub fn print(&self) -> Result<()> {
    self.print_files(&self.inner)
  }

  /// Print the file tracing information.
  pub fn print_file(&self, path: &Path, lang: ThreadLang) -> Result<()> {
...
impl<W: Write + Sync> TraceInfo<RuleTrace, W> {
  ...
  pub fn print_file(&self, path: &Path, lang: ThreadLang, rules: &[&RuleConfig<ThreadLang>]) -> Result<()> {
    self.print_entity("file", path.display(), |w| {
      let len = rules.len();
  ...
  pub fn print_rules(&self, rules: &RuleCollection<ThreadLang>) -> Result<()> {
...
mod test {
  ...
  fn test_tracing() {
    ...
    assert_eq!(
      ret,
      "thread: summary|file: scannedFileCount=0,skippedFileCount=0\n"
    );
    ...
    assert_eq!(
      ret,
      r"thread: summary|file: scannedFileCount=0,skippedFileCount=0
thread: summary|rule: effectiveRuleCount=10,skippedRuleCount=2
"
    );
  }
