...
#[cfg(any(feature = "language", feature = "dynamic-language"))]
...
fn register_impl(regs: LanguageGlobs) -> Result<Vec<(ThreadLang, Types)>> {
  let mut lang_globs = vec![];
  for (lang, globs) in regs {
    let lang = ThreadLang::from_str(&lang).with_context(|| EC::UnrecognizableLanguage(lang))?;
...
/// Merges multiple `Types` into a single `Types`.
...
html: ['*.vue', '*.svelte']";
  ...
  #[test]
  fn test_merge_types() {
    let lang: ThreadLang = thread_languages::SupportedLanguage::Rust.into();
  ...
  fn test_merge_with_globs() -> Result<()> {
    ...
    let lang: ThreadLang = thread_languages::SupportedLanguage::Html.into();
    let default_types = lang.file_types();
