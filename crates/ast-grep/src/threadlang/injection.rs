...
use crate::{DeserializeEnv, RuleCore, SerializableRuleCore};
#[cfg(all(feature="tree-sitter", feature="language"))]
use crate::{LanguageExt, StrDoc, TSRange, Doc, Node};
...
#[derive(#[cfg(feature = "serde")] Serialize, #[cfg(feature = "serde")] Deserialize, Clone)]
#[serde(untagged)]
pub enum Injected {
  Static(String),
  Dynamic(Vec<String>),
...
#[cfg(feature = "ag-config")]
fn merge_default_injectable(ret: &mut HashMap<ThreadLang, Injection>) {
...
fn register_injectable(
  ...
  injectable: &mut HashMap<ThreadLang, Injection>,
) -> Result<()> {
...
#[cfg(all(feature = "ag-config", feature = "tree-sitter", feature = "language"))]
...
fn node_to_range<D: Doc>(node: &Node<D>) -> TSRange {
  ...
  let sp = crate::Point::new(sp.0, sp.1);
  let end = node.end_pos();
  let ep = end.byte_point();
  let ep = crate::Point::new(ep.0, ep.1);
  TSRange {
    start_byte: r.start,
    end_byte: r.end,
    start_point: sp,
...
injected: [js, ts, tsx]";
  ...
  fn test_good_injection() {
    ...
    let lang = ThreadLang::from(thread_languages::SupportedLanguage::JavaScript);
