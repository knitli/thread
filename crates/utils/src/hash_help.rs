...
pub fn hash_file_with_seed(file: &mut std::fs::File, seed: u64) -> Result<u64, std::io::Error> {
    ...
    rapidhash::v3::rapidhash_v3_file_seeded(file, &secrets).map_err(std::io::Error::other)
}
...
mod tests {
    ...
    fn test_hash_file_vs_hash_bytes_consistency() -> Result<(), std::io::Error> {
        ...
        assert_eq!(
            ...
            "File hash should match byte hash for same content"
        );
        Ok(())
    }
    ...
    fn test_hash_file_with_seed_vs_hash_bytes_consistency() -> Result<(), std::io::Error> {
        ...
        assert_eq!(
            file_hash, bytes_hash,
            "Seeded file hash should match seeded byte hash"
        );
        Ok(())
    }
