...
use thread_utilities::{RapidMap, RapidSet, map_with_capacity};
...
struct ScanResultInner<'t, D: Doc> {
    ...
    matches: RapidMap<usize, Vec<NodeMatch<'t, D>>>,
...
struct Suppressions {
    ...
    lines: RapidMap<usize, Suppression>,
}

impl Suppressions {
    fn collect_all<D: Doc>(root: &AstGrep<D>) -> (Self, RapidMap<usize, Node<'_, D>>) {
        let mut suppressions = Self {
            file: None,
            lines: RapidMap::default(),
        };
        let mut suppression_nodes = RapidMap::default();
    ...
    fn collect<'r, D: Doc>(
        &mut self,
        node: &Node<'r, D>,
        suppression_nodes: &mut RapidMap<usize, Node<'r, D>>,
    ) -> bool {
...
struct Suppression {
    /// None = suppress all
    suppressed: Option<RapidSet<String>>,
    node_id: usize,
}
...
fn parse_suppression_set(text: &str) -> Option<RapidSet<String>> {
...
mod test {
    ...
    use thread_ast_engine::tree_sitter::{LanguageExt, StrDoc};
