...
) -> FastMap<String, (L, SerializableRuleCore)> {
  rules
    .into_iter()
...
struct TopologicalSort<'a, T: DependentRule> {
  maps: &'a FastMap<String, T>,
  order: Vec<&'a str>,
  // bool stands for if the rule has completed visit
  seen: FastMap<&'a str, bool>,
}

impl<'a, T: DependentRule> TopologicalSort<'a, T> {
  fn get_order(maps: &FastMap<String, T>) -> OrderResult<Vec<&str>> {
  ...
  fn new(maps: &'a FastMap<String, T>) -> Self {
    Self {
      maps,
      order: vec![],
      seen: FastMap::new(),
...
impl<L: Language> DeserializeEnv<L> {
  ...
  pub fn with_utils(
    self,
    utils: &FastMap<String, SerializableRule>,
  ...
  pub(crate) fn get_transform_order<'a>(
    &self,
    trans: &'a FastMap<String, Trans<MetaVariable>>,
