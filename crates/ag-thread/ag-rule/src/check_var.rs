...
pub fn check_rule_with_hint<'r>(
  ...
  constraints: &'r FastMap<String, Rule>,
...
fn check_utils_defined(rule: &Rule, constraints: &FastMap<String, Rule>) -> RResult<()> {
  rule.verify_util()?;
...
fn get_vars_from_rules<'r>(rule: &'r Rule, utils: &'r RuleRegistration) -> FastSet<&'r str> {
...
fn check_var_in_constraints<'r>(
  mut vars: FastSet<&'r str>,
  constraints: &'r FastMap<String, Rule>,
) -> RResult<FastSet<&'r str>> {
...
fn check_var_in_fix(vars: FastSet<&str>, fixers: &Vec<Fixer>) -> RResult<()> {
  for fixer in fixers {
...
pub use {CheckHint, check_rule_with_hint, check_rewriters_in_transform};
