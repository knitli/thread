...
impl Suppressions {
    fn collect_all<D: Doc>(root: &AstGrep<D>) -> (Self, FastMap<usize, Node<'_, D>>) {
        let mut suppressions = Self {
            file: None,
            lines: FastMap::new(),
        };
        let mut suppression_nodes = FastMap::new();
    ...
    fn collect<'r, D: Doc>(
        &mut self,
        node: &Node<'r, D>,
        suppression_nodes: &mut FastMap<usize, Node<'r, D>>,
    ) -> bool {
...
fn parse_suppression_set(text: &str) -> Option<FastSet<String>> {
