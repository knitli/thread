...
impl SerializableRuleCore {
    ...
    fn get_deserialize_env<L: Language>(
        &self,
    ...
    fn get_constraints<L: Language>(
        &self,
        env: &DeserializeEnv<L>,
    ) -> RResult<FastMap<String, Rule>> {
        let mut constraints = FastMap::new();
...
impl RuleCore {
    ...
    #[inline]
    pub fn with_matchers(self, constraints: FastMap<String, Rule>) -> Self {
        Self {
            constraints,
            ..self
        }
    ...
    pub(crate) fn defined_node_vars(&self) -> FastSet<&str> {
...
mod test {
    ...
    use ag_service_core::matcher::{Pattern, RegexMatcher};
  ...
  B: {substring: {source: $A}}",
    ...
    fn test_rule_reg_with_utils() {
        ...
        let ser_rule: SerializableRuleCore =
            from_str("{rule: {matches: test}, utils: {test: {kind: number}} }")
                .expect("should deser");
    ...
    fn get_rewriters() -> (&'static str, RuleCore) {
        ...
        let rewriter: SerializableRuleCore =
            from_str("{rule: {kind: number, pattern: $REWRITE}, fix: yjsnp}")
                .expect("should parse");
