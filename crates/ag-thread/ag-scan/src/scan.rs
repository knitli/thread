...
use ag_service_rule::{read_rule_file, with_rule_stats, ProjectConfig};
use thread_lang::ThreadLang;
...
use ag_service_utils::RuleOverwrite;
use ag_service_utils::{filter_file_rule, ContextArgs, InputArgs, OutputArgs, OverwriteArgs};
use ag_service_utils::{FileTrace, ScanTrace};
use ag_service_utils::{Items, PathWorker, StdInWorker, Worker};
...
struct ScanWithConfig {
    arg: ScanArg,
    configs: RuleCollection<ThreadLang>,
    unused_suppression_rule: RuleConfig<ThreadLang>,
    trace: ScanTrace,
    // TODO: remove this
...
fn unused_suppression_rule_config(
    arg: &ScanArg,
    overwrite: &RuleOverwrite,
) -> RuleConfig<ThreadLang> {
    let severity = overwrite
        .find("unused-suppression")
        .severity
        .unwrap_or_else(|| default_unused_suppression_rule_severity(arg));
    CombinedScan::unused_config(severity, SupportedLanguage::Rust.into())
}
...
struct ScanStdin {
    rules: Vec<RuleConfig<ThreadLang>>,
    // TODO: remove this
    error_count: AtomicUsize,
}
impl ScanStdin {
...
fn match_rule_diff_on_file<T>(
    path: &Path,
    matches: Vec<(&RuleConfig<ThreadLang>, NodeMatch<StrDoc<ThreadLang>>)>,
...
fn match_rule_on_file<T>(
    path: &Path,
    matches: Vec<NodeMatch<StrDoc<ThreadLang>>>,
...
"#;
    ...
    fn test_run_with_config() {
        ...
        file.write_all("fn test() { Some(123) }".as_bytes())
            .unwrap();
        file.sync_all().unwrap();
    ...
    fn test_scan_with_inline_rules_error() {
        ...
        let err =
            run_with_config(arg, Err(anyhow::anyhow!("not found"))).expect_err("should error");
        assert!(err.is::<EC>());
