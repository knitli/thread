...
pub fn filter_file_rule(
    ...
    if let Some(injected) = lang.injectable_sg_langs() {
        let sub_roots = grep.get_injections(|s| ThreadLang::from_str(s).ok());
...
pub fn filter_file_pattern<'a>(
    ...
    sub_matchers: &'a [(ThreadLang, Pattern)],
    ...
    let injections = grep.get_injections(|s| ThreadLang::from_str(s).ok());
...
mod test {
    ...
    fn test_html_embedding() {
        let root = ThreadLang::Builtin(SupportedLanguage::Html)
            .ast_grep("<script lang=typescript>alert(123)</script>");
        let docs = root.get_injections(|s| ThreadLang::from_str(s).ok());
    ...
    fn test_html_embedding_lang_not_found() {
        let root =
            ThreadLang::Builtin(SupportedLanguage::Html).ast_grep("<script lang=xxx>alert(123)</script>");
        let docs = root.get_injections(|s| ThreadLang::from_str(s).ok());
