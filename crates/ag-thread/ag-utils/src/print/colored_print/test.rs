...
fn test_print_matches() {
  let lang = SgLang::from(SupportedLanguage::Tsx);
...
fn test_print_rules() {
  ...
  let lang = SgLang::from(SupportedLanguage::TypeScript);
  for &(source, pattern, note) in MATCHES_CASES {
...
fn get_printed_text(mut printer: ColoredPrinter<Buffer>, diff_case: &DiffCase) -> String {
  ...
  let globals = GlobalRules::default();
  let grep = SgLang::from(SupportedLanguage::TypeScript).ast_grep(source);
  let rule = from_yaml_string(
    &format!(
      r"
id: test-id
