...
use ag_service_label::{LabelStyle};
use ag_service_rule::{Metadata, RuleConfig, Severity};
use ag_service_ast::{Doc, Node as SgNode};
use ag_service_pattern::{MetaVariable, NodeMatch};
...
struct MetaVariables<'t> {
  single: FastMap<String, MatchNode<'t>>,
  multi: FastMap<String, Vec<MatchNode<'t>>>,
  transformed: FastMap<String, String>,
}
fn from_env<'t>(nm: &NodeMatch<'t>) -> Option<MetaVariables<'t>> {
  let env = nm.get_env();
  let mut vars = env.get_matched_variables().peekable();
  vars.peek()?;
  let mut single = FastMap::new();
  let mut multi = FastMap::new();
  let mut transformed = FastMap::new();
  for var in vars {
...
fn get_range(n: &Node<'_, ThreadLang>) -> Range {
...
fn get_labels<'b, 't>(rule: &'b RuleConfig<ThreadLang>, nm: &NodeMatch<'t>) -> Vec<MatchLabel<'t, 'b>> {
  rule
    .get_labels(nm)
    .into_iter()
    .map(|label| {
...
impl<'t, 'b> RuleMatchJSON<'t, 'b> {
  fn new(nm: NodeMatch<'t>, path: &'b str, rule: &'b RuleConfig<ThreadLang>, metadata: bool) -> Self {
  ...
  fn diff(diff: Diff<'t>, path: &'b str, rule: &'b RuleConfig<ThreadLang>, metadata: bool) -> Self {
...
impl PrintProcessor<Buffer> for JSONProcessor {
  ...
  fn print_rule_diffs(
    &self,
    diffs: Vec<(Diff, &RuleConfig<ThreadLang>)>,
    path: &Path,
  ) -> Result<Buffer> {
    let path = path.to_string_lossy();
    let jsons = diffs
...
mod test {
  ...
  fn test_invariant() {
    for &(source, pattern, _, note) in MATCHES_CASES {
      ...
      let grep = ThreadLang::from(SupportedLanguage::Tsx).ast_grep(source);
  ...
  fn test_replace_json() {
    for &(source, pattern, replace, note) in MATCHES_CASES {
      ...
      let lang = ThreadLang::from(SupportedLanguage::Tsx);
...
";
  ...
  fn test_transform() {
    ...
    let grep = ThreadLang::from(SupportedLanguage::TypeScript).ast_grep("console.log(123)");
    let matches = grep.root().find_all(&rule.matcher).collect();
    printer.before_print().unwrap();
    let buffer = printer
      .get_processor()
