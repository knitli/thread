...
use crate::print_diff::DiffStyles;
...
fn sg_label_to_code_span_label(label: ag_service_label::Label<impl Doc>) -> Label<()> {
  let ret = match label.style {
    ag_service_label::LabelStyle::Primary => Label::primary((), label.range()),
    ag_service_label::LabelStyle::Secondary => Label::secondary((), label.range()),
  };
