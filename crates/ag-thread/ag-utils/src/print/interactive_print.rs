...
mod test {
  ...
  use ag_service_fix::Fixer;
  use ag_service_tree_sitter::{StrDoc, Visitor, AstGrep};
...
{rule}"
  ...
  fn make_diffs(grep: &AstGrep<StrDoc<ThreadLang>>, matcher: impl Matcher, fixer: &Fixer) -> Diffs<()> {
    let root = grep.root();
  ...
  fn test_apply_rewrite() {
    let root = AstGrep::new("let a = () => c++", SupportedLanguage::TypeScript.into());
...
fix: ($B, lifecycle.update(['$A']))",
  ...
  fn test_rewrite_nested() {
    let root = AstGrep::new("Some(Some(1))", SupportedLanguage::TypeScript.into());
    let diffs = make_diffs(
      &root,
      "Some($A)",
      &Fixer::from_str::<ThreadLang>("$A", &SupportedLanguage::TypeScript.into())
  ...
  fn test_rewrite_with_empty_lines() {
    let root = AstGrep::new("\n\n\nSome(1)", SupportedLanguage::TypeScript.into());
    let diffs = make_diffs(
      &root,
      "Some($A)",
      &Fixer::from_str::<ThreadLang>("$A", &SupportedLanguage::TypeScript.into())
