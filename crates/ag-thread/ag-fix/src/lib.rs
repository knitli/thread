use ag_service_rule::{Relation, Rule, RuleSerializeError, StopBy};
use crate::transform::transformation::Transformation;
use crate::DeserializeEnv;
use ag_service_transform::replacer::{Content, Replacer, TemplateFix, TemplateFixError};
use ag_service_types::{SerializableFixer, SerializableFixConfig, FixerError, Expansion, Fixer};
use ag_service_ast::{Doc, Language, Matcher, NodeMatch};
use ag_service_rule::maybe::Maybe;
...
pub use {Fixer, Expansion, Transformation};
...
impl Fixer {
    fn do_parse<L: Language>(
        ...
        env: &DeserializeEnv<L>,
        transform: &Option<FastMap<String, Transformation>>,
    ...
    pub(crate) fn used_vars(&self) -> FastSet<&str> {
