...
fn apply_snapshot_action(
    ...
    path_map: FastMap<String, PathBuf>,
) -> Result<()> {
...
fn verify_test_case_simple<'a>(
    ...
    rules: &RuleCollection<ThreadLang>,
    snapshots: Option<&SnapshotCollection>,
) -> Option<CaseResult<'a>> {
...
"
    ...
    pub fn get_rule_config(rule: &str) -> RuleConfig<ThreadLang> {
        let globals = GlobalRules::default();
        let inner = from_str(&get_rule_text(rule)).unwrap();
        RuleConfig::try_from(inner, &globals).unwrap()
    }
    fn always_report_rule() -> RuleCollection<ThreadLang> {
    ...
    fn never_report_rule() -> RuleCollection<ThreadLang> {
