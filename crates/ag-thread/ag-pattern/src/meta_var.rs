...
use ag_service_types::{MetaVarEnv, MetaVariableID, Content, Doc, Node}
use crate::Matcher::Matcher;
use std::borrow::Cow;
use thread_utils::FastMap;

use {MetaVarEnv, MetaVariable, MetaVariableID};
...
impl<'t, D: Doc> MetaVarEnv<'t, D> {
  pub fn new() -> Self {
    Self {
      single_matched: FastMap::new(),
      multi_matched: FastMap::new(),
      transformed_var: FastMap::new(),
    }
  }
  ...
  pub fn match_constraints<M: Matcher>(
    &mut self,
    var_matchers: &FastMap<MetaVariableID, M>,
  ) -> bool {
...
impl<'tree, D: Doc> From<MetaVarEnv<'tree, D>> for FastMap<String, String> {
...
pub mod metavar {
  pub use super::{MetaVarEnv, MetaVariable, MetaVariableID};
}
