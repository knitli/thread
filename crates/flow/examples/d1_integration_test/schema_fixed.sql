...
-- Indexes for common queries (SQLite doesn't support inline INDEX syntax)
-- FIXED: Separate CREATE INDEX statements instead of inline INDEX in CREATE TABLE
CREATE INDEX IF NOT EXISTS idx_file_path ON code_symbols(file_path);
CREATE INDEX IF NOT EXISTS idx_symbol_name ON code_symbols(symbol_name);
CREATE INDEX IF NOT EXISTS idx_symbol_type ON code_symbols(symbol_type);
