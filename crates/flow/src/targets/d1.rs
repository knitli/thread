...
impl D1ExportContext {
    ...
    async fn execute_sql(
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| RecocoError::internal_msg(format!("D1 API request failed: {}", e)))?;
        ...
        let result: serde_json::Value = response
            .json()
            .await
            .map_err(|e| RecocoError::internal_msg(format!("Failed to parse D1 response: {}", e)))?;
    ...
    fn build_delete_stmt(&self, key: &KeyValue) -> Result<(String, Vec<serde_json::Value>), RecocoError> {
...
fn key_part_to_json(key_part: &recoco::base::value::KeyPart) -> Result<serde_json::Value, RecocoError> {
...
fn value_to_json(value: &Value) -> Result<serde_json::Value, RecocoError> {
    Ok(match value {
...
fn basic_value_to_json(value: &BasicValue) -> Result<serde_json::Value, RecocoError> {
    Ok(match value {
...
fn value_type_to_sql(value_type: &ValueType) -> String {
    match value_type {
