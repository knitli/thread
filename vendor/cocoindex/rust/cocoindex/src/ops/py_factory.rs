    ...
    fn call_py_fn<'py>(
        ...
        let result = self
            ...
            .call(
                ...
                kwargs
                    .map(|kwargs| -> Result<_> { kwargs.into_py_dict(py).from_py_result() })
                    .transpose()?
                    .as_ref(),
            )
            .from_py_result()
...
impl interface::SimpleFunctionExecutor for Arc<PyFunctionExecutor> {
    async fn evaluate(&self, input: Vec<value::Value>) -> Result<value::Value> {
        ...
        let result_fut = Python::attach(|py| -> Result<_> {
            ...
            from_py_future(py, &task_locals, result_coro).from_py_result()
        })?;
...
impl interface::SourceExecutor for PySourceExecutor {
    ...
    async fn get_value(
        ...
        let py_result = Python::attach(|py| -> Result<_> {
            ...
            from_py_future(py, &task_locals, result_coro.into_bound(py)).from_py_result()
        })?
        .await;
    ...
    async fn next_partial_source_row(
        ...
        let next_item_coro = Python::attach(|py| -> Result<_> {
            let coro = py_async_iter
                ...
                .from_py_result()
                .with_context(|| "while iterating over user-configured source".to_string())?;
            let task_locals =
                pyo3_async_runtimes::TaskLocals::new(py_exec_ctx.event_loop.bind(py).clone());
...
impl interface::TargetFactory for PyExportTargetFactory {
    ...
    fn check_state_compatibility(
        ...
        let compatibility = Python::attach(|py| -> Result<_> {
            let result = self
                ...
                .from_py_result()
                .with_context(|| {
                    "while calling check_state_compatibility in user-configured target".to_string()
                })?;
    ...
    fn describe_resource(&self, key: &serde_json::Value) -> Result<String> {
        Python::attach(|py| -> Result<String> {
            let result = self
                ...
                .from_py_result()
                .with_context(|| {
                    "while calling describe_resource in user-configured target".to_string()
                })?;
    ...
    async fn apply_setup_changes(
        ...
        let py_result = Python::attach(move |py| -> Result<_> {
            let result_coro = self
                ...
                .from_py_result()
                .with_context(|| {
                    "while calling apply_setup_changes_async in user-configured target".to_string()
                })?;
            let task_locals =
                pyo3_async_runtimes::TaskLocals::new(py_exec_ctx.event_loop.bind(py).clone());
            from_py_future(py, &task_locals, result_coro.into_bound(py)).from_py_result()
        ...
        Python::attach(move |_py| {
            py_result
                .from_py_result()
                .with_context(|| "while applying setup changes in user-configured target".to_string())
        })?;
    ...
    async fn apply_mutation(
        ...
        Python::attach(move |_py| {
            py_result
                .from_py_result()
                .with_context(|| "while applying mutations in user-configured target".to_string())
        })?;
        Ok(())
    }
}
