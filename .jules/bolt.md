...
## 2026-04-10 - [Performance: Dynamic SQL Generation]
**Learning:** For dynamic SQL generation (e.g., in `crates/flow/src/targets/d1.rs` for Cloudflare D1 targets), using `String::with_capacity` and the `write!` macro (via `std::fmt::Write`) to construct queries directly instead of intermediate `Vec` allocations and `format!` in loops minimizes heap allocations and string copies.
**Action:** Always use `String::with_capacity` and `write!` macro to build query strings directly.
