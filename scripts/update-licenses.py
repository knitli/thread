...
def parse_args() -> ArgumentParser:
    ...
    parser = ArgumentParser(description="Update licenses for files in the repository.")
    parser.add_argument(
        "--files",
        nargs="+",
        default=[],
...
ALLOWED_DIRS = [d for d in Path.cwd().iterdir() if d.is_dir() and (not d.name.startswith(".") or d.name in {".github", ".vscode", ".roo", ".claude"})]

def is_allowed_path(path: Path) -> bool:
    """Check if the path is allowed based on the allowed directories."""
    if any(path.is_relative_to(allowed_dir) for allowed_dir in ALLOWED_DIRS) and path.is_file():
        return path in files if (files := parse_args().files) else True
    return False

BASE_CMD = ["reuse", "annotate", "--year", str(datetime.now().year), "--copyright", "Knitli Inc. <knitli@knit.li>", "--fallback-dot-license", "--skip-existing"]

AST_GREP_COPYRIGHT = "Herrington Darkholme <2883231+HerringtonDarkholme@users.noreply.github.com>"

AST_GREP_PATHS = [path for path in (list((Path.cwd() / "crates" / "ast-grep").rglob("*.rs")) + list((Path.cwd() / "crates" / "languages").rglob("*.rs"))) if path.is_file() and is_allowed_path(path)]

CODE_EXTS = {"rs", "sh", "py", "js", "ts", "tsx", "jsx", "java", "go", "c", "cpp", "h", "hpp", "html", "css", "svelte", "vue"}
...
NON_CODE_PATHS = [
    p for ext in NON_CODE_EXTS
    for p in Path.cwd().rglob(
        f"*.{ext}"
    )
    if p.is_file() and p not in AST_GREP_PATHS and
    is_allowed_path(p)
]
print(f"Found {len(AST_GREP_PATHS)} AST-Grep paths, {len(CODE_PATHS)} code paths, and {len(NON_CODE_PATHS)} non-code paths.")
...
def main() -> None:
    ...
    if not AST_GREP_PATHS and not CODE_PATHS and not NON_CODE_PATHS:
        return
    if AST_GREP_PATHS:
        ast_grep_cmd = BASE_CMD + ["-c", AST_GREP_COPYRIGHT, "-l", "MIT"] + AST_GREP_PATHS
        run_command(ast_grep_cmd, AST_GREP_PATHS)
    if CODE_PATHS:
        code_cmd = BASE_CMD + ["-l", "AGPL-3.0-or-later"] + CODE_PATHS
        run_command(code_cmd, CODE_PATHS)
    if NON_CODE_PATHS:
        non_code_cmd = BASE_CMD + ["-l", "MIT OR Apache-2.0"] + NON_CODE_PATHS
